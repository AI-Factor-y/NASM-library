...
section .bss
	...
	highest_occur_elem :resw 1
	lowest_occur_elem : resw 1
	high_occur_val : resw 1
	low_occur_val :resw 1
	dup_count : resw 1
	curr_num: resw 1
...
section .text
	...
	mov word[highest_occur_elem],0
	mov word[lowest_occur_elem],0
	mov word[low_occur_val],100
	mov word[high_occur_val],0
	...
	loop1:
		...
		; check high value 
		mov ax,word[high_occur_val]
		
		cmp word[dup_count],ax
		jbe no_change_high
			mov ax,word[dup_count]
			mov word[high_occur_val],ax
			mov ax,word[curr_num]
			mov word[highest_occur_elem],ax

		no_change_high:

		; check low value
		mov ax,word[low_occur_val]
		
		cmp word[dup_count],ax
		jae no_change_low
			mov ax,word[dup_count]
			mov word[low_occur_val],ax
			mov ax,word[curr_num]
			mov word[lowest_occur_elem],ax
	...
	pop rax	
	; print highest
	mov ax,word[highest_occur_elem]
