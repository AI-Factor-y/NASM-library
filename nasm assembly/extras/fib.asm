...
section .text
	...
	cmp word[limit],1 ; if 1 then print 1 only
	je exit

	mov eax,2

	mov word[first_num],1
	mov word[prev_num], 1

	loop1:
		mov cx,word[first_num]


			mov word[num],cx
			call print_num

		add cx,word[prev_num]

		mov bx, word[first_num]
		mov word[prev_num],  bx

		mov word[first_num],cx
